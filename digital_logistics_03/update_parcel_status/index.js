...
const sendFacebookMessage = (parcel_id, customer, status) => {
    ...
    if (FB_ACCESS_TOKEN === undefined) {
        ...
        if (fb_psid === undefined) {
            ...
            return fbReply(fb_psid, messages, FB_ACCESS_TOKEN)
                .then(res => {
    ...
    event.Records.forEach((record) => {
        if (record.eventName === 'INSERT') {
            ...
            if (hasAnyStatusChanged(oldStatus, newStatus)) {
                ...
                return findFbPsid(record.dynamodb.NewImage.parcel_customer_id.S)
                    .then(data => {
                        if (!data.Items.length) {
                            console.log(`Customer not found: ${record.dynamodb.NewImage.parcel_customer_id.S}`)
                        } else {
                            return sendFacebookMessage(record.dynamodb.NewImage.parcel_customer_id.S, data, newStatus)
                        }
                    })
